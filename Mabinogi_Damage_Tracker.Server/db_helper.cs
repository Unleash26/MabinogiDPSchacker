...
namespace Mabinogi_Damage_tracker
{
    public class db_helper
    {
        ...
        /// </summary>
        public static List<object> Get_DamageBySkill_GroupedByPlayers_BetweenUT(int start_ut, int end_ut)
        {
            try
            {
                using (SqliteConnection connection = new SqliteConnection(db_connection))
                {
                    connection.Open();
                    SqliteCommand command = new SqliteCommand(@"
                        SELECT damages.playerid, playername, skill, SUM(damage) as total_damage
                        FROM damages
                        LEFT JOIN players ON damages.playerid = players.playerid
                        WHERE ut BETWEEN @start_ut AND @end_ut
                        GROUP BY damages.playerid, skill
                        ORDER BY damages.playerid, total_damage DESC
                    ", connection);

                    command.Parameters.AddWithValue("@start_ut", start_ut);
                    command.Parameters.AddWithValue("@end_ut", end_ut);

                    // プレイヤーごとのスキル別ダメージをまとめる
                    var playerSkillData = new Dictionary<long, List<object>>();
                    ...
                    using (SqliteDataReader reader = command.ExecuteReader())
                    {
                        ...
                        {
                            long playerId = reader.GetInt64(reader.GetOrdinal("playerid"));
                            string playerName = reader.IsDBNull(reader.GetOrdinal("playername")) 
                                ? $"{playerId}" 
                                : reader.GetString(reader.GetOrdinal("playername"));
                            int skillId = reader.GetInt32(reader.GetOrdinal("skill"));
                            double totalDamage = reader.GetDouble(reader.GetOrdinal("total_damage"));
                            ...
                            if (!playerSkillData.ContainsKey(playerId))
                            {
                                playerSkillData[playerId] = new List<object>();
                            }

                            playerSkillData[playerId].Add(new
                            {
                                skillId = skillId,
                                skillName = skillName,
                                damage = totalDamage
                    ...
                    foreach (var kvp in playerSkillData)
                    {
                        result.Add(new
                        {
                            playerId = kvp.Key,
            ...
            {
                Debug.WriteLine("Get_DamageBySkill_GroupedByPlayers_BetweenUT error: " + ex.Message);
                return null;
            }
        }
        ...
        /// </summary>
        public static object Get_DamageBySkill_AfterId(int lastFetchedId)
        {
            try
            {
                using (SqliteConnection connection = new SqliteConnection(db_connection))
                {
                    connection.Open();
                    SqliteCommand command = new SqliteCommand(@"
                        SELECT MAX(damages.id) as max_id, damages.playerid, playername, skill, SUM(damage) as total_damage
                        FROM damages
                        LEFT JOIN players ON damages.playerid = players.playerid
                        WHERE damages.id > @lastFetchedId
                        GROUP BY damages.playerid, skill
                    ...
                    using (SqliteDataReader reader = command.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            long maxId = reader.GetInt64(reader.GetOrdinal("max_id"));
                    ...
                    return new { lastId = lastId, data = data };
                }
            }
            catch (Exception ex)
            {
                Debug.WriteLine("Get_DamageBySkill_AfterId error: " + ex.Message);
                return null;
            }
        }

        /// <summary>
        /// スキル別の総合ダメージを取得（プレイヤー関係なく）- 円グラフ用
        /// </summary>
        public static List<object> Get_TotalDamageBySkill_BetweenUT(int start_ut, int end_ut)
        {
            try
            {
                using (SqliteConnection connection = new SqliteConnection(db_connection))
                {
                    connection.Open();
                    SqliteCommand command = new SqliteCommand(@"
                        SELECT skill, SUM(damage) as total_damage
                        FROM damages
                        WHERE ut BETWEEN @start_ut AND @end_ut
                        GROUP BY skill
            ...
            {
                Debug.WriteLine("Get_TotalDamageBySkill_BetweenUT error: " + ex.Message);
                return null;
            }
        }
